...
{
    public partial class FrmBresenham : BaseFrm
    {

        private CircularBuffer2<Point> points = new CircularBuffer2<Point>();
        private BresenhamGrapher bresenhamGrapher = new BresenhamGrapher();
        public FrmBresenham() : base("Bresenham Line Drawer")
        {
            InitializeComponent();
        ...
        {
            bresenhamGrapher.ReadData(txtX1, txtY1, txtX2, txtY2);
            bresenhamGrapher.DrawAsync(canvas);

            listPoints.Items.Clear();
            listPoints.Items.Add("N  X        Y");
            int index = 0;
            foreach (var punto in bresenhamGrapher.GetPoints())
            {
